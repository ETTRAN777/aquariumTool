...
// tank yet). Corrects a task's dueDate against startDate before
// anything else in this file ever sees it.
//
// Recurring tasks get walked forward through their own cadence to find
// the first honest occurrence on/after startDate — same endDate-as-
...
// relative to startDate mathematically would need a *tracked prior*
// startDate to compute a delta against, which only exists if one was
// ever recorded, and often won't be; not worth building for a case that
// only partially covers itself. null here means "needs a new date
// picked by hand," never "no date at all" — the caller's job is to
// treat it as a real, distinct case, not silently drop it.
export function effectiveDueDate(task: ScheduleTask, startDate: string | undefined): string | null {
  if (!startDate || task.dueDate >= startDate) return task.dueDate;
...
export function pickMostRelevantTask(schedule: ScheduleTask[], startDate?: string): ScheduleTask | undefined {
  const withEffectiveDate = schedule
    .filter((t) => !t.done)
    .map((t) => ({ task: t, effective: effectiveDueDate(t, startDate) }))
